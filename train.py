...
vae = VAE().to(device)  # Move model to device
...
# Create fixed latent vectors on the correct device
latent_dim = 64  # Make sure this matches your VAE latent dimension
fixed_z = torch.randn(64, latent_dim).to(device)

def save_sample_images(epoch, fixed_z, folder='runs/images', num_images=10):
    with torch.no_grad():
        # Move fixed_z to device in case it isn’t already
        fixed_z = fixed_z.to(device)
        samples = vae.decoder(fixed_z).cpu()  # Move to CPU for saving
